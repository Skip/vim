...
" syntax region mipsComment start="\/\*" skip="\/\*" end="\*\/"
"syntax match mipsCommentStartError display "/\*"me=e-1 contained
"syntax region mipsComment start="/\*" end="\*/" contains=mipsCommentStartError extend

"hi def link mipsCommentStart mipsComment
"hi def link mipsError Error
"hi def link mipsSpaceError mipsError
"hi def link mipsCommentStartError mipsError

"syn keyword	mipsTodo contained TODO FIXME XXX

"syn cluster	mipsCommentGroup contains=mipsTodo




"syntax region mipsComment matchgroup=mipsCommentStart start="\/\*[*]" end="\*\/" contains=@mipsCommentGroup,mipsCommentStartError,mipsSpaceError extend
"syntax region mipsComment start="/\*" end="\*/"
"syntax region mipsComment start="/\*" end="\*/"
" syntax region mipsComment start="\*/" skip="\\$" end="/\*"
...
"syntax region mipsComment start="//" skip="\\$" end="$"
"syntax region mipsComment start="//" end="$"
"syn region mipsComment start=/^\s*\/\// end=/$/

"syntax region mipsComment start="\#if\ 0" end="\#endif"
syntax region mipsDefine  start="^\s*\(%:\|#\)\s*\(define\|undef\|ifdef\|else\|endif\|elif\|error\|warning\|line\)\>" skip="\\$" end="$" end="//"me=s-1 keepend contains=ALLBUT

syn region mipsIncluded	display contained start=+"+ skip=+\\\\\|\\"+ end=+"+
syn match mipsIncluded display contained "<[^>]*>"
syn match mipsInclude display "^\s*\(%:\|#\)\s*include\>\s*["<]" contains=mipsIncluded
...
syn region  mipsComment		 start="/\*"  end="\*/" contains=@javaCommentSpecial,javaTodo,@Spell
syn match   javaCommentStar      contained "^\s*\*[^/]"me=e-1
syn match   javaCommentStar      contained "^\s*\*$"
syn match   mipsLineComment      "//.*" contains=@javaCommentSpecial2,javaTodo,@Spell
...
hi def link mipsLineComment    Comment
